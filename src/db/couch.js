...
export default config => {
  let protocol = config.protocolSecure ? 'https' : 'http'
  return {
    setConfig(newConfig){
      config = newConfig
      protocol = config.protocolSecure ? 'https' : 'http'
    },
    loadType(type) {
      const sType = serialise(type)
      return fetch(
        `${protocol}://${config.name}:${config.pass}@${config.couchUrl}/${config.dataDB}/_design/docs/_view/type?key=${sType}`, {
          method: 'GET',
        }).then(res => res.json())
    },
    load(_id) {
      return fetch(
        `${protocol}://${config.name}:${config.pass}@${config.couchUrl}/${config.dataDB}/${_id}`, {
          method: 'GET',
        }).then(res => res.json())
    },
    ...
    uploadAttachment(doc, file) {
      return fetch(`${protocol}://${config.name}:${config.pass}@${config.couchUrl}/${config.dataDB}/${doc._id}/${file.name}?rev=${doc._rev}`, {
        method: 'PUT',
        body: file,
      })
        .then(res => res.json())
    ...
    createDb() {
      return fetch(`${protocol}://${config.name}:${config.pass}@${config.couchUrl}/${config.dataDB}`, { method: 'PUT' })
    },
    deleteDb() {
      return fetch(`${protocol}://${config.name}:${config.pass}@${config.couchUrl}/${config.dataDB}`, { method: 'DELETE' })
    }
  }
}
